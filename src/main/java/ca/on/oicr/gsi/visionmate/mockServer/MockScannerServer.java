 ...
 * </pre>
 * <p>
 * When run, the server waits for a connection, will remain connected as long as the client stays, and will shut down when the 
...
public class MockScannerServer implements Runnable {
  ...
  /**
   * Sets the server configuration. This must be matched identically by the client to ensure proper function
   * 
   * @param config
   */
  ...
  /**
   * Formats the response data to conform to the configuration settings, and formatting of the real server's output
   * 
   * @param request
   * @param response
   * @return
