...
public class ScanStatus {
  ...
  /**
   * Constructs a ScanStatus from the byte representation of the 8 status bits. Note that Java bytes are signed, so int 128 = byte -127, 
   * int 255 = byte -1, etc. Neither the integer nor the whole byte value need be maintained, as the byte is read as 8 separate bits for 
  ...
  /**
   * @return true if this ScanStatus indicates the scanner has scan data ready for retrieval; false otherwise
   */
  ...
  /**
   * @return true if this ScanStatus indicates the scanner's current scan data has been retrieved from the server at least once
   */
  ...
  /**
   * @return true if this ScanStatus indicates the scanner's current configured product has 96 positions; false otherwise. This is only 
   * an indication of the scanner's configuration, and not the physical presence of such a rack
   */
