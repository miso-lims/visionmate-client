...
public class VisionMateClient implements AutoCloseable {
  ...
  private void write(String command) {
    if (!telnet.isConnected()) throw new IllegalStateException("Client is not connected.");
  ...
  /**
   * Waits for a response from the server and reads until the prompt (suffix character). This is a blocking call and does not return 
   * until the response is received, or the socket timeout is reached
   * 
  ...
  /**
   * Sends a command and waits for the response. This is a blocking call and does not return until the response is received, or the 
   * socket timeout is reached
   * 
   * @param command text to send
   * @return the useful response data, which is the server's response, stripped of things such as prefix and suffix characters, 
   * acknowledgement text, and command echo
   * @throws SocketTimeoutException if wait for response times out
  ...
  /**
   * Requests the scanner's status from the server. This is a blocking call and does not return until the response is received, or the 
   * socket timeout is reached
   * 
  ...
  /**
   * Requests the current rack type information from the server. This is a blocking call and does not return until the response is 
   * received, or the socket timeout is reached
   * 
  ...
  /**
   * Sets the expected rack type information. This is a blocking call and does not return until the response is 
   * received, or the socket timeout is reached
   * 
  ...
  /**
   * Resets the scanner's status. This marks its scanning, finished scan, data ready, data sent, and error statuses all to false. 
   * Useful when intending to wait for a change (e.g. reset and wait for data ready before retrieving scan to ensure that a new 
   * scan is being retrieved, and not a previous one)
   * 
  ...
  public boolean resetStatus() throws SocketTimeoutException, IOException {
    ...
    // configured timeout will be used to choose how long to wait for status change verification. The resulting timeout will be this, 
    // plus the time it takes for each call. An individual call is more likely to time out than the status change though.
    for (int i = timeout / statusPollingDelay; i > 0; i--) {
      ...
      if (!status.isDataReady()) return true;
    }
    return false;
  }
  
  /**
   * Retrieves the most recent scan results from the server. Caution: old results are saved indefinitely. If you want to ensure that 
   * you are retrieving a new scan, it is best to call {@link #resetStatus()} and then wait until new data is ready before calling this 
   * method. Alternately, see the {@link #waitForScan()} and {@link #prepareAndWaitForScan()} methods
   * 
   * @return the scan data, including all rack position data, or null if there is no available scan data
   * @throws SocketTimeoutException if wait for response times out
   * @throws IOException if any other read/write errors occur in communication
  ...
  public Scan getScan() throws SocketTimeoutException, IOException, ScannerException {
    ScanStatus status = getStatus();
    if (status.isError()) throw new ScannerException(getLastError());
    if (!status.isDataReady()) return null;
  ...
  /**
   * Waits for the server to report data ready, then retrieves scan results from the server. It is ideal to call {@link #resetStatus()} 
   * before this method to ensure that old data is not mistakenly retrieved. Alternately, see the {@link #prepareAndWaitForScan()} method. 
   * This is a blocking call and does not return until the status has changed and scan results have been retrieved, or the maxWaitTime has 
   * elapsed
   * 
   ...
   * Waits for the server to report data ready, then retrieves scan results from the server. It is ideal to call {@link #resetStatus()} 
   * before this method to ensure that old data is not mistakenly retrieved. Alternately, see the {@link #prepareAndWaitForScan()} method. 
   * This is a blocking call and does not return until the status has changed and scan results have been retrieved, or the default wait 
   * time has elapsed
   * 
   * @return the scan data, including all rack position data, or null if there is no available scan data
   * @throws SocketTimeoutException if wait for response times out
  ...
  /**
   * Resets the scanner status, waits for the server to report data ready, then retrieves scan results from the server. This ensures 
   * that data retrieved is from a scan that runs while this method is running. This is a blocking call and does not return until the 
   * status has changed and scan results have been retrieved, or the maxWaitTime has elapsed
   * 
   * @param maxWaitTime milliseconds to wait for data to be ready
   * @return the scan data, including all rack position data, or null if there is no available scan data
   * @throws SocketTimeoutException if wait for response times out
   ...
   * Resets the scanner status, waits for the server to report data ready, then retrieves scan results from the server. This ensures 
   * that data retrieved is from a scan that runs while this method is running. This is a blocking call and does not return until the 
   * status has changed and scan results have been retrieved, or the default wait time has elapsed
   * 
   * @return the scan data, including all rack position data, or null if there is no available scan data
   * @throws SocketTimeoutException if wait for response times out
   * @throws IOException if any other read/write errors occur in communication
