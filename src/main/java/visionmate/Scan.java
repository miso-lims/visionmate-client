...
public class Scan {
   ...
   * @param config server configuration used when creating the data String. This is used check the delimiter and labels for No Tube 
   * (empty position) and No Read (failure to scan a tube)
   */
  ...
  /**
   * @return the label that represents a position where there is a tube, but the barcode could not be read
   */
  ...
  /**
   * Gets a single barcode from the scan. Positions are 1-based, so the top-left position would be row 1, column 1
   * 
   * @param row
   * @param column
  ...
  /**
   * Gets a single barcode from the scan. Column number is 1-based, so the top-left position is row A, column 1
   * 
   * @param row
   * @param column
   * @return the barcode at the specified position
  ...
  /**
   * @return true if every position has a tube in it; false otherwise. Note: a return value of true does not mean that all barcodes 
   * were read successfully. See also {@link #getNoReadCount()}
   */
  ...
  /**
   * Converts a row letter into the equivalent row number for referencing a Scan row. Note that row numbers are 1-based, so A=1.
   * 
   * @param letter
